//هه

import 'package:flutter/material.dart';
import 'package:arabic_letters/item.dart/listmenu.dart';
import 'package:arabic_letters/screen.dart/bottonbar.dart';





class haha extends StatelessWidget {
  const haha({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Directionality(
       textDirection: TextDirection.rtl,
       child: Scaffold(



    bottomNavigationBar:
    mybotton(nameafterletter: 'حرف ب',
      namebeforeletter: 'الصفحه الرئيسيه',
      namemenuletter:  'الصفحه الرئيسيه',
      ontap1: (){
        Navigator.of(context).pushNamed('/wwww'); 
      },
      ontap2: (){
        Navigator.of(context).pushNamed('/homepage');
      },
      ontap3: (){
        Navigator.of(context).pushNamed('/nnnn');
      },
      ),



         appBar: AppBar(
           backgroundColor: Colors.lime,
     title:const Text(' حرف   ه ',style: TextStyle(fontWeight: FontWeight.bold,color: Colors.black),) ,
     centerTitle: true,
     
    
     
         ),
     
     

         body: 

//   listmenu  هنا استدعيت فنكشن ال 

           listmenu(
               nameletter: ' ه',
            //   pictureletter: 'asset/ass-1.png',
               pictureoneletter: 'https://assets10.lottiefiles.com/private_files/lf30_lsg0irlz.json',
               wordoneletter: ' هديه',
               //wordallletter: 'ديه',
               picturetwoletter: 'https://assets9.lottiefiles.com/private_files/lf30_WIWuRl.json',
               wordtwoletter: ' هرم',
              // wordalltwoletter: 'رم',
               picturethreeletter: 'https://assets8.lottiefiles.com/packages/lf20_0dwuFO.json',
               wordthreeletter: ' هلال',
             //  wordallthreeletter: 'لال',
           
             ),



       ),
     );
  }
}